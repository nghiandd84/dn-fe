<script lang="ts">
	...
	const taggingResources = $derived<string[]>(data.taggingResources || []);

	const allNavItems = $derived([
		{ href: '/admin', label: $LL.admin_panel.nav.dashboard(), resource: null },
		{ href: '/admin/tag-groups', label: $LL.admin_panel.nav.tag_groups(), resource: 'tag-groups' },
		{ href: '/admin/tags', label: $LL.admin_panel.nav.tags(), resource: 'tags' },
		{ href: '/admin/entity-tags', label: $LL.admin_panel.nav.entity_tags(), resource: 'entity-tags' }
	]);

	const navItems = $derived(
		allNavItems.filter((item) => !item.resource || taggingResources.includes(item.resource))
	);
