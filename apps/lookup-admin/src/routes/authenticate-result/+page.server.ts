...
function onPermissions(permissions: Permission[], cookies: Cookies, clientId: string, port: string | URL) {
	...
	const lookupPerms = permissions.filter((p) => p.resource.startsWith('LOOKUP:'));
	const lookupResources = lookupPerms
	...
	if (lookupResources.length > 0) {
		...
		for (const p of lookupPerms) {
			const resource = LOOKUP_RESOURCE_MAP[p.resource];
