...
function onPermissions(permissions: Permission[], cookies: Cookies, clientId: string, port: string | URL) {
	...
	const profilePerms = permissions.filter((p) => p.resource.startsWith('PROFILE:'));
	const profileResources = profilePerms
	...
	if (profileResources.length > 0) {
		...
		for (const p of profilePerms) {
			const resource = PROFILE_RESOURCE_MAP[p.resource];
