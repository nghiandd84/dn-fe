<script lang="ts">
	...
	const allNavItems = $derived([
		...
		{ href: '/admin/urls', label: $LL.admin_panel.nav.urls(), resource: 'urls' },
		{ href: '/admin/api-keys', label: $LL.admin_panel.nav.api_keys(), resource: 'api-keys' },
		{ href: '/admin/url-clicks', label: $LL.admin_panel.nav.url_clicks(), resource: 'url-clicks' }
	]);

	const navItems = $derived(
		allNavItems.filter((item) => !item.resource || urlShortenerResources.includes(item.resource))
	);
