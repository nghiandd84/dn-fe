...
function onPermissions(permissions: Permission[], cookies: Cookies, clientId: string, port: string | URL) {
	...
	const urlShortenerPerms = permissions.filter((p) => p.resource.startsWith('URL_SHORTENER:'));
	const urlShortenerResources = urlShortenerPerms
	...
	if (urlShortenerResources.length > 0) {
		...
		for (const p of urlShortenerPerms) {
			const resource = URL_SHORTENER_RESOURCE_MAP[p.resource];
