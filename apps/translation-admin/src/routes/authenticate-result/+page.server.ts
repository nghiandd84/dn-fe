...
const TRANSLATION_RESOURCE_MAP: Record<string, string> = {
	'TRANSLATION:PROJECT': 'projects',
	'TRANSLATION:TAG': 'tags',
	'TRANSLATION:KEY': 'translation-keys',
	'TRANSLATION:VERSION': 'translation-versions'
};
...
function onPermissions(permissions: Permission[], cookies: Cookies, clientId: string, port: string | URL) {
	...
	const translationPerms = permissions.filter((p) => p.resource.startsWith('TRANSLATION:'));
	const translationResources = translationPerms
		.map((p) => TRANSLATION_RESOURCE_MAP[p.resource])
		.filter(Boolean);
	if (translationResources.length > 0) {
		cookies.set(`${prefix}resources`, JSON.stringify(translationResources), {
			path: '/',
			httpOnly: true,
			sameSite: 'lax',
			secure: false,
		...
		for (const p of translationPerms) {
			const resource = TRANSLATION_RESOURCE_MAP[p.resource];
